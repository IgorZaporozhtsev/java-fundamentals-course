...
//                .max((account1, account2) -> account1.getBalance().intValue());
        ...
        return accounts.stream()
                .collect(groupingBy(
                        account -> account.getEmail()
                                .substring(account.getEmail().lastIndexOf("@"))
                                .replaceAll("@", "")
                ));
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                ...
                .map(mail -> mail.substring(mail.lastIndexOf("@")).replaceAll("@", ""))
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream()
                .collect(
                        groupingBy(Account::getLastName, mapping(Account::getFirstName, toSet())));
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
                .collect(groupingBy(a -> a.getCreationDate().getMonth(),
                        //mapper - what we want to map
                        //collector - how we want to map
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                ...
                .filter(n -> n.length() >= nameLengthBound)
                .flatMapToInt(String::chars)
                .mapToObj(c -> (char) c)
                .collect(groupingBy(Function.identity(), counting()));
    }
